...
uint8_t usart2_dma_buffer[30];

// =============================================================================
void taskPrint(void) {
    for (int i = 0; i < sizeof_array(usart2_dma_buffer); i++) {
        console.printf("%02X ", usart2_dma_buffer[i]);
    }
    console.printf("\r\n");
...
int main(void) {
    ...
    usart2.ring.show('h', 10);
    // setup ringbuffer, using DMA -----------
    usart2.dma.config(usart2_dma_buffer, sizeof_array(usart2_dma_buffer));
    usart2.ring.show('h', 10);
